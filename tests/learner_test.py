...
class LearnerTests(unittest.TestCase):
    ...
    def test_basic_resolution(self):
        ...
        self.learner.receive_accepted(
            AckValueMessage('B', ProposalID('B', 10), Value(42)))
        assert self.learner.learned_value is None
        self.learner.receive_accepted(
            AckValueMessage('C', ProposalID('B', 10), Value(42)))
        assert self.learner.learned_value == 42

    def test_ignore_duplicate_messages(self):
        accept_message = AckValueMessage('B', ProposalID('B', 10), Value(42))
    ...
    def test_learner_ignores_accept_when_already_learned(self):
        ...
        self.learner.receive_accepted(
            AckValueMessage('C', ProposalID('B', 10), Value(42)))
        self.learner.receive_accepted(
            AckValueMessage('D', ProposalID('A', 12), Value(44)))
        assert self.learner.learned_value == 42

    def test_learner_does_not_learn_without_quorum(self):
        self.learner.receive_accepted(
            AckValueMessage('A', ProposalID('A', 10), Value(42)))
        self.learner.receive_accepted(
            AckValueMessage('B', ProposalID('B', 10), Value(10)))
